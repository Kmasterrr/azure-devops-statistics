...
param 
(
    [string]$OutputPath = (Join-Path (Split-Path $PSScriptRoot -Parent) "data"),
...
$PAT = $env:ADOS_PAT
$OutputBasePath = if ($env:LOCAL_DATA_PATH) { $env:LOCAL_DATA_PATH } else { Join-Path (Split-Path $PSScriptRoot -Parent) "data" }
...
try {
    ...
    Write-Host "`nCollecting Projects..." -ForegroundColor Yellow
    ...
    Write-Host "`nCollecting Git Repositories..." -ForegroundColor Yellow
    $allRepos = @()
    ...
    Write-Host "`nCollecting Git Commits..." -ForegroundColor Yellow
    $allCommits = @()
    ...
    Write-Host "`nCollecting Builds..." -ForegroundColor Yellow
    $allBuilds = @()
    foreach ($project in $projects) {
    ...
    Write-Host "`nCollecting Work Items..." -ForegroundColor Yellow
    $allWorkItems = @()
    foreach ($project in $projects) {
