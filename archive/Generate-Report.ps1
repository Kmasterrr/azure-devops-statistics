...
if (-not $DataPath) {
    ...
    $BaseDataPath = Join-Path (Join-Path (Split-Path $PSScriptRoot -Parent) "data") $Organization
...
"@
    ...
    if ($LoadedData.GitRepositories) {
        ...
        <table>
            <tr><th>Project</th><th>Repository Name</th><th>Default Branch</th><th>Size (bytes)</th></tr>
"@
        foreach ($repo in ($LoadedData.GitRepositories.data | Sort-Object project, name)) {
            ...
            $htmlContent += "<tr><td>$($repo.project)</td><td>$($repo.name)</td><td>$branch</td><td>$size</td></tr>"
        }
        $htmlContent += "</table>"
    }
