...
if (-not $Organization) {
    Write-Host "Please set ADOS_ORGANIZATION environment variable or run Setup-Environment.ps1 first" -ForegroundColor Red
    exit 1
}

$BaseDataPath = Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) "data\$Organization"
if (Test-Path $BaseDataPath) {
    ...
    $commitsFile = Join-Path $LatestDate.FullName "GitCommits.json"
} else {
    Write-Host "No data found. Please run .\scripts\Collect-Statistics.ps1 first" -ForegroundColor Red
    exit 1
}
