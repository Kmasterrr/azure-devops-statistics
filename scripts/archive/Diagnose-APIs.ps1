...
try {
    ...
    foreach ($project in $projects) {
        ...
        Write-Host "`nTesting Release Definitions..." -ForegroundColor Cyan
        try {
            $releaseDefs = Get-AzDevOpsReleaseDefinitionList -Session $AzSession -Project $project.name -Top 100
            Write-Host "✅ Found $($releaseDefs.Count) release definitions" -ForegroundColor Green
        }
        catch {
            Write-Host "❌ Release Definitions Error: $($_.Exception.Message)" -ForegroundColor Red
        }
