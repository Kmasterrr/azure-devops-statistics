...
Write-Host "🏆 Azure DevOps Top Contributors Analysis" -ForegroundColor Cyan
...
if (Test-Path $commitsFile) {
    ...
    Write-Host "`n📊 Top 10 Contributors by Commit Count:" -ForegroundColor Green
    ...
    foreach ($contributor in $topContributors) {
        ...
        Write-Host "    Commits: $($contributor.Count) ($percentage% of total)" -ForegroundColor Gray
        Write-Host "    Latest: $latestDate" -ForegroundColor Gray
        Write-Host ""
        $rank++
    }
    
    Write-Host "`n📈 Activity Summary:" -ForegroundColor Green
    ...
    Write-Host "`n🏢 Contributors by Project:" -ForegroundColor Green
    ...
    Write-Host "`n📚 Most Active Repositories:" -ForegroundColor Green
